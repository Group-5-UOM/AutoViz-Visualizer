...
def test_parallel_gates_resume_without_an_interrupt_id(registry, nulls_id):
    ...
    out = agent.run("fare by cls, and separately age by cls, dropping missing", dataset_id=nulls_id)
...
def test_different_decisions_queue_and_do_not_cross_apply(registry, nulls_id):
    ...
    first = agent.run("fare by cls, and separately age by cls, dropping missing", dataset_id=nulls_id)
...
def test_cleaning_choices_on_different_columns_queue_separately(registry, tmp_path):
    ...
    first = agent.run("salary by dept, and separately salary by region", dataset_id=ds)
