"""Auth routes — register / login / logout / me (Bearer-token sessions).
    ...
    POST /auth/register   Create an account                  -> {id, email}
    POST /auth/login      Exchange credentials for a token   -> {access_token, ...}
    POST /auth/logout     Revoke the presented token
    GET  /auth/me         The authenticated user

The token is opaque (not a JWT): `repository.create_token` stores it in the
`sessions` table with an expiry, so logout revokes it server-side immediately.
"""
